...
function paginate_str_btn($current_page,$total_pages)
{
    ...
    if($total_pages > 0  && $current_page <= $total_pages){ //verify total pages and current page number
        ...
        if($current_page > 1){//กรณี current active page มากกว่า 1 ให้แสดง first page และ previous page
            ...
            $pagination .= '<li class="first"><a href="#" data-page="1" title="First" class="btn btn-lg">&laquo;</a></li>'; //first link
            $pagination .= '<li><a href="#" data-page="'.$previous_link.'" title="Previous" class="btn btn-lg">&lt;</a></li>'; //previous link
            $chkleft = $current_page-2;
                for($i = $chkleft; $i < $current_page; $i++){ //Create left-hand side links
                    if($i > 0){
                        $pagination .= '<li><a href="#" data-page="'.$i.'" title="Page'.$i.'" class="btn btn-lg">'.$i.'</a></li>';
                    }
                }   
        ...
        for($j = $next; $j < $right_links ; $j++){ 
            if($j<=$total_pages){
                $pagination .= '<li><a href="#" data-page="'.$j.'" title="Page '.$j.'" class="btn btn-lg">'.$j.'</a></li>';
            }
        }
        if($current_page < $total_pages){ 
				$next_link = ($next > $total_pages) ? $total_pages : $next;
                $pagination .= '<li><a href="#" data-page="'.$next_link.'" title="Next" class="btn btn-lg">&gt;</a></li>'; //next link
                $pagination .= '<li class="last"><a href="#" data-page="'.$total_pages.'" title="Last" class="btn btn-lg">&raquo;</a></li>'; //last link
        }
