...
$conn = new mysqli(DB_SERVER,DB_USER,DB_PASS,DB_NAME);
    ...
    gogov_new.officialdoc_num, gogov_new.gogov_for, gogov_new.gogov_topic, gogov_new.gogov_real_date1, gogov_new.gogov_real_date2, gogov_new.json_print,gogov_new.id as gogov_new_id, gogov_new.datecreate
    FROM gogov_new 
    RIGHT JOIN person ON gogov_new.person_id = person.id 
    ...
    WHERE concat_ws(gogov_new.gogov_topic, gogov_new.gogov_for,  gogov_new.officialdoc_num, person.cid, person.fname, person.lname, person.birth_date, position.position_name, class_position.class_position_type_name2, department.dep_name, groupwork.groupwork_name, government_emp_type.government_emp_type_name)
    like '%%{$data}%%' group by gogov_new.id order by gogov_new.status_use = 'Y' AND person.record_use = 'Y' AND person.cid != null , gogov_new.id DESC")) or die("aaaa");
